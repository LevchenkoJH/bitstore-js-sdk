...
class Aes {
    ...
    */
    static decrypt_with_checksum(
        ...
        if (private_key && public_key)
            S = private_key.get_shared_secret(public_key, legacy);
        else if (secret)
            S = secret;
        else
            throw new Error("decrypt_with_checksum does not contain secret");
        
        // D E B U G
        // if (private_key && public_key)
        //     console.log('D E B U G decrypt_with_checksum', {
    ...
    */
    static encrypt_with_checksum(private_key = null, public_key = null, secret = null, nonce, message) {
        // Warning: Do not put `nonce = ""` in the arguments, in es6 this will not convert "null" into an emtpy string
        ...
        if (private_key && public_key)
            ...
            throw new Error("encrypt_with_checksum does not contain secret");

        // D E B U G
        // if (private_key && public_key)
        //     console.log('D E B U G encrypt_with_checksum', {
        //         priv_to_pub: private_key.toPublicKey().toString(),
        //         pub: public_key.toPublicKeyString(),
        //         nonce: nonce,
        //         message: message.length,
