...
class TransactionBuilder {
    constructor(tx = null, data_blocks = null) {
        this.signer_private_keys = [];

        if (tx) {
            this.expiration = data_blocks && tx.expiration < (60 * 30 + base_expiration_sec()) ? (60 * 30 + base_expiration_sec()) : tx.expiration;
            ...
            this.expiration = data_blocks ? (60 * 30 + base_expiration_sec()) : 0;
            this.operations = [];
            this.signatures = [];
        }
    ...
    set_required_fees(asset_id, removeDuplicates) {
        ...
        for (let i = 0, op; i < this.operations.length; i++) {
            ...
            try {
                ...
                */
                console.log('isProposal(op)', isProposal(op));
                if (isProposal(op)) {
